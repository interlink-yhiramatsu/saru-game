...
for(var F=0;F<a;F++){var D=C[F];D.sleep();}for(var E=0;E<i;E++){var G=f[E];G.sleep();}createjs.Ticker.addEventListener("tick",A);v.stage.addEventListener("stagemousedown",t);
v.stage.addEventListener("stagemouseup",z);v.visible=true;}function t(D){k(p.mc.x,p.mc.y);o=true;}function z(D){o=false;}function k(E,D){for(var F=0;F<i;
F++){var G=f[F];if(G.getIsReady()==true){if(saru.Utils.getRandom(2)==0){G.activate(E,D,0);}else{G.activate(E,D,1);}break;}}}function b(){for(var F=0;F<a;
F++){var D=C[F];if(D.getIsReady()==true){var E=saru.Utils.getRandom(6);if(E==0){D.spawn(-10,saru.Utils.getRandom(saru.Const.HEIGHT),saru.Const.TYPE_DEF);
...
B();}function B(){clearInterval(u);createjs.Ticker.removeEventListener("tick",A);v.stage.removeEventListener("stagemousedown",t);v.stage.removeEventListener("stagemouseup",z);
}function q(D){v.dispatchEvent(new Event(saru.Const.MAIN_END));}function c(){}function e(){return l;}return y;};